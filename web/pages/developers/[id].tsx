...
const GRADE_STYLES: Record<string, { color: string; bg: string; border: string }> = {
  ...
  D: { color: 'text-slate-500',   bg: 'bg-slate-50',   border: 'border-slate-200'   },
};
...
function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <Link
      ...
      className="group bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md hover:border-slate-300 transition-all flex flex-col gap-3"
    >
      ...
      <div className="flex-1">
        <h3 className="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1">{listing.name}</h3>
        {listing.description && (
          <p className="text-slate-500 text-sm mt-1 line-clamp-2">{listing.description}</p>
        )}
      </div>
      <div className="flex items-center justify-between text-sm">
        <span className="font-bold text-slate-900">${(listing.price_cents / 100).toFixed(2)}</span>
        {listing.download_count > 0 && (
          <span className="text-slate-400">{listing.download_count} installs</span>
        )}
      </div>
    </Link>
  );
    ...
    return (
      <div className="min-h-screen bg-cream-100">
        ...
        <div className="max-w-xl mx-auto px-6 py-24 text-center">
          <h1 className="text-2xl font-bold text-slate-900 mb-4">Developer not found</h1>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 mb-8">
            <div className="flex flex-col sm:flex-row items-start gap-6">
              {/* Avatar */}
              {avatarSrc ? (
                <img src={avatarSrc} alt={developer.name} className="w-24 h-24 rounded-2xl object-cover flex-shrink-0 border border-slate-200" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  {developer.is_pro && (
                    <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                ...
                {developer.bio && <p className="text-slate-600 mb-3 leading-relaxed">{developer.bio}</p>}

                {/* Social links */}
                <div className="flex flex-wrap gap-4 text-sm">
                  {developer.website && (
                    <a href={developer.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-blue-600 hover:underline">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                      Website
                    </a>
                  )}
                  {developer.github && (
                    <a href={`https://github.com/${developer.github}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-slate-600 hover:text-slate-900">
                  ...
                  {developer.twitter && (
                    <a href={`https://x.com/${developer.twitter}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-slate-600 hover:text-slate-900">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {collections.map(c => (
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1">{c.name}</h3>
          ...
          {listings.length === 0 ? (
            <div className="text-center py-12 text-slate-500">No published listings yet.</div>
          ) : (
