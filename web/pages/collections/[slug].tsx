  ...
  return (
    <div className="card overflow-hidden hover:shadow-md transition-all">
      <Link href={`/listings/${item.product.slug}`} className="block p-5">
        ...
        <h3 className="font-semibold text-ink-900 line-clamp-1 mb-1">{item.product.name}</h3>
        {item.product.description && (
          <p className="text-ink-500 text-sm line-clamp-2">{item.product.description}</p>
        )}
        <div className="flex items-center justify-between mt-3 text-sm">
          <span className="font-bold text-ink-900">${(item.product.price_cents / 100).toFixed(2)}</span>
          {item.product.download_count > 0 && (
            <span className="text-ink-400">{item.product.download_count} installs</span>
          )}
        </div>
      </Link>
      {item.note && (
        <div className="px-5 py-3 bg-cream-200/60 border-t border-cream-200">
          <p className="text-xs text-ink-500 italic">"{item.note}"</p>
        </div>
      )}
    </div>
  );
  ...
  if (error || !collection) return (
    <div className="min-h-screen bg-cream-100">
      ...
      <div className="max-w-xl mx-auto px-6 py-24 text-center">
        <h1 className="heading-serif text-2xl text-ink-900 mb-4">Collection not found</h1>
      ...
      <main className="pt-20 pb-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-ink-400">{collection.items.length} listing{collection.items.length !== 1 ? 's' : ''}</span>
                </div>
                <h1 className="heading-serif text-2xl text-ink-900 mb-2">{collection.name}</h1>
                {collection.description && (
                  <p className="text-ink-600 leading-relaxed">{collection.description}</p>
                )}
              </div>
            </div>

            <div className="flex items-center gap-3 mt-6 pt-6 border-t border-cream-200">
              <Link href={ownerHref} className="flex items-center gap-3 group">
                {collection.owner.avatar_url ? (
                  <img src={collection.owner.avatar_url.startsWith('/avatars/') ? `${API_URL}${collection.owner.avatar_url}` : collection.owner.avatar_url}
                    alt={collection.owner.name} className="w-8 h-8 rounded-full object-cover border border-cream-200" />
                ) : (
                ...
                <span className="text-sm font-medium text-ink-700 group-hover:text-terra-600 transition-colors">
          ...
          {collection.items.length === 0 ? (
            <div className="text-center py-12 text-ink-500">No listings in this collection yet.</div>
          ) : (
